...
interface ReviewListItem {
  ...
  // lets a reviewer see review progress on a submission without opening it.
...
export interface ReviewQueueViewProps {
   ...
   *  "/assistant-director-planning" — used to build the per-row "Review" link. */
  basePath: string;
  /** Which status this queue considers "needs my attention" by default. */
  defaultStatusFilter: SubmissionStatus;
  /** The subset of statuses this reviewer's queue can ever contain, plus "all". */
  statusFilterOptions: StatusFilterOption[];
}

export function ReviewQueueView({ basePath, defaultStatusFilter, statusFilterOptions }: ReviewQueueViewProps) {
  const { lang } = useLanguage();
  const { data, isLoading } = useSWR(`/api/submissions?year=${CURRENT_YEAR}&limit=100`, fetcher);

  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<SubmissionStatus | "all">(defaultStatusFilter);
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-right">
                      <Button asChild size="sm" variant="outline" className="touch-target gap-1.5">
                        <Link href={`${basePath}/review/${item.id}`}>
