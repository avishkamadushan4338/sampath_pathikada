...
interface SubmissionDetail {
  ...
  /** Resolves each `reviewedById` found in `sectionReviews` to a display name/role — the JSON
   *  column only stores the user id, so the API attaches this lookup alongside the raw data. */
  reviewerNames?: Record<string, { name: string; role: string }>;
}
...
export interface ReviewDetailViewProps {
   ...
   *  "/assistant-director-planning" — used for the back link and post-decision redirect. */
  basePath: string;
  /** Statuses at which this submission has left the current reviewer's queue and the page
   *  should navigate back to the queue list. AD's decision is "done" once the submission reaches
   *  AD_APPROVED (handed off to DS) or REJECTED; DS's is "done" at APPROVED or REJECTED. */
  terminalStatuses: SubmissionStatus[];
}

export function ReviewDetailView({ divisionId, basePath, terminalStatuses }: ReviewDetailViewProps) {
        ...
        {SECTION_KEYS.map((key) => (
          <TabsContent key={key} value={key} className="rounded-xl border border-border bg-card p-4 sm:p-6">
            <SectionReviewBar
              ...
              sectionKey={key}
              review={reviews[key]}
              reviewer={reviews[key] ? submission.reviewerNames?.[reviews[key]!.reviewedById] : undefined}
