...
export interface ReviewerSummaryViewProps {
  /** Which status this reviewer stage considers "awaiting my review" — SUBMITTED for AD,
   *  AD_APPROVED for DS. Drives the stage-breakdown chart and the approval-rate/turnaround stats,
   *  which only make sense relative to this reviewer's own decisions. */
  awaitingReviewStatus: Extract<SubmissionStatus, "SUBMITTED" | "AD_APPROVED">;
}

export function ReviewerSummaryView({ awaitingReviewStatus }: ReviewerSummaryViewProps) {
  ...
  const stageBreakdown = useMemo(() => {
    ...
    const counts = {
      ...
      awaitingReview: rows.filter((r) => r.status === awaitingReviewStatus).length,
    ...
    return [
      ...
      { label: STATUS_LABEL[awaitingReviewStatus][lang], value: counts.awaitingReview, fill: GOLD },
  ...
  }, [data, gnRoster, lang, awaitingReviewStatus]);
