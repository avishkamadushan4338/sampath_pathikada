...
function makeRow(i: number, overrides: Record<string, unknown> = {}) {
  return {
    ...
    data: {},
    submittedBy: { name: `Officer ${i}`, email: `officer${i}@example.com` },
    ...overrides,
  };
}
...
describe("GET /api/export/excel — authentication", () => {
  it("rejects a missing session", async () => {
    mockGetSession.mockResolvedValue(null);
    const res = await GET(getRequest(`${APP_URL}/api/export/excel`));
...
describe("GET /api/export/excel — authorization", () => {
  it.each([
  ...
  it("scopes ADMIN's export to their own dsDivision regardless of query params (division-scoping / IDOR attempt)", async () => {
    ...
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?dsDivision=matara-fg&district=matara`));
    expect(res.status).toBe(200);
  ...
  it("lets SUPER_ADMIN scope by dsDivision or district query params", async () => {
    ...
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?district=matara`));
    expect(res.status).toBe(200);
    const where = mockPrisma.submission.findMany.mock.calls[0][0].where;
...
describe("GET /api/export/excel — validation", () => {
  it("defaults to CURRENT_YEAR when no year is given (valid input)", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    await GET(getRequest(`${APP_URL}/api/export/excel`));
    expect(mockPrisma.submission.findMany.mock.calls[0][0].where.year).toBe(2026);
  });

  it("accepts an explicit valid year", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    await GET(getRequest(`${APP_URL}/api/export/excel?year=2025`));
    expect(mockPrisma.submission.findMany.mock.calls[0][0].where.year).toBe(2025);
  });

  it("parses a comma-separated gnDivisions filter (unexpected fields elsewhere are ignored)", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    await GET(getRequest(`${APP_URL}/api/export/excel?gnDivisions=${encodeURIComponent("3-1-39-005, 3-1-39-010")}&bogus=1`));
    const where = mockPrisma.submission.findMany.mock.calls[0][0].where;
  ...
  it("handles a non-numeric year (malformed input) without crashing", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?year=not-a-year`));
    expect(res.status).toBe(200);
  });
});

describe("GET /api/export/excel — row cap / large dataset protection", () => {
  it("requests take: EXPORT_ROW_CAP + 1, never an unbounded findMany", async () => {
    ...
    await GET(getRequest(`${APP_URL}/api/export/excel?year=2026`));
    expect(mockPrisma.submission.findMany.mock.calls[0][0].take).toBe(EXPORT_ROW_CAP + 1);
  });

  it("returns 413 when the result exceeds the cap, without generating a workbook", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    ...
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?year=2026`));
    expect(res.status).toBe(413);
    expect(res.headers.get("content-type")).not.toContain("spreadsheetml");
  });

  it("succeeds at exactly the cap boundary", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    const atCap = Array.from({ length: EXPORT_ROW_CAP }, (_, i) => makeRow(i));
    mockPrisma.submission.findMany.mockResolvedValue(atCap);
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?year=2026`));
    expect(res.status).toBe(200);
  }, 30_000);
});

describe("GET /api/export/excel — empty datasets", () => {
  it("generates a valid workbook response for zero rows", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    mockPrisma.submission.findMany.mockResolvedValue([]);
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?year=2026`));
    expect(res.status).toBe(200);
...
describe("GET /api/export/excel — generation, content type, filename", () => {
  it("returns the correct Excel content type", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
  ...
  it("sets a Content-Disposition attachment header with a scoped filename", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    mockPrisma.submission.findMany.mockResolvedValue([makeRow(1)]);
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?year=2026&dsDivision=galle-fg`));
    const disposition = res.headers.get("content-disposition");
    expect(disposition).toContain("attachment");
    expect(disposition).toContain("division-summary-galle-fg-2026.xlsx");
  });
  ...
  it("uses 'all' as the scope slug when unscoped (SUPER_ADMIN, no filters)", async () => {
    ...
    mockPrisma.submission.findMany.mockResolvedValue([makeRow(1)]);
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?year=2026`));
    expect(res.headers.get("content-disposition")).toContain("division-summary-all-2026.xlsx");
  });

  it("produces a non-empty binary body for normal data (generation correctness smoke test)", async () => {
    mockGetSession.mockResolvedValue(SUPER_ADMIN_SESSION);
    mockPrisma.submission.findMany.mockResolvedValue([makeRow(1), makeRow(2), makeRow(3)]);
    const res = await GET(getRequest(`${APP_URL}/api/export/excel?year=2026`));
    const buf = Buffer.from(await res.arrayBuffer());
    // XLSX (zip) files start with the "PK" local file header signature.
