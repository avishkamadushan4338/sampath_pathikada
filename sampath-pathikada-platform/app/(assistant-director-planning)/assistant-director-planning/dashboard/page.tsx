...
/** /api/audit-logs auto-scopes an Assistant Director Planning's query server-side (their own
 *  division, DATA category, WARNING/ERROR severity only) regardless of what's passed here — this
 *  just asks for the most recent handful to show as a compact dashboard flag, not a full log
 *  browser. */
...
export default function AssistantDirectorPlanningDashboardPage() {
  const { lang } = useLanguage();
  ...
  const counts = useMemo(() => {
    ...
    // AD's "awaiting review" bucket is SUBMITTED (the first reviewer stage) — AD_APPROVED and
    // beyond means AD already finished with it.
    const submitted = rows.filter((r) => r.status === "SUBMITTED").length;
    const adApproved = rows.filter((r) => r.status === "AD_APPROVED").length;
    ...
    const pending = Math.max(0, gnRoster.length - (submitted + adApproved + approved + rejected + revisionNeeded));
    return { submitted, adApproved, approved, rejected, revisionNeeded, pending };
  }, [data, gnRoster]);
  ...
  const statCards: {
    ...
    { key: "adApproved", value: counts.adApproved, label: STATUS_LABEL.AD_APPROVED, icon: CheckCircle2, colorVar: "--status-pending" },
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        <h1 className="font-display text-fluid-3xl font-semibold text-primary">
          {dsDivision ? (lang === "si" ? dsDivision.si : dsDivision.en) : (
            <Bilingual en="Assistant Director Planning Dashboard" si="සහකාර සැලසුම් අධ්‍යක්ෂ පාලක පුවරුව" />
          )}
        </h1>
      ...
      <div className="mb-8 grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-4">
        <Link href="/assistant-director-planning/review" className="block focus-visible:outline-none">
        ...
        <Link href="/assistant-director-planning/summary" className="block focus-visible:outline-none">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(clamp(220px,28vw,300px),1fr))] gap-4">
          {displayedRoster.map((gn) => {
            ...
            const colorVar = !submission
              ? "--border"
              : submission.status === "SUBMITTED" || submission.status === "AD_APPROVED" || submission.status === "REVISION_NEEDED"
                ? "--status-pending"
                  ...
                  {submission ? (
                    <>
                      ...
                      <Button asChild size="sm" variant="outline" className="touch-target w-fit gap-1.5">
                        <Link href={`/assistant-director-planning/review/${submission.id}`}>
